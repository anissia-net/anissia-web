<template>
  <div class="bg-white dark:bg-black text-zinc-800 dark:text-zinc-400">
    <div class=" container pb-10 m-auto px-1 sm:px-2 md:px-4 duration-300 min-h-screen py-0" @click="e => evtClickClosePopup(e)">

      <!-- 타이틀 -->
      <div class="title font-extralight h-16 text-center flex items-center">
        <div class="title-main text-2xl leading-none flex-1 text-center pl-9"><a href="/" target="_blank">애니<span class="mob-hide">메이션</span> 편성표</a></div>
      ...
      <div class="fixed inset-0 backdrop-blur-sm bg-gray-800/10" v-if="animeNow != null">
        <div class="box w-[450px] mt-[150px] mb-0 mx-auto p-4 rounded-md shadow-lg text-base font-semibold text-center bg-zinc-50/90 dark:bg-zinc-900/90 text-gray-800 dark:text-zinc-300">
          ...
          <!-- 요일 -->
          <div class="text-base mt-3" v-if="animeNow.period">{{animeNow.period}}</div>
          <!-- 자막제작자 목록 -->
          <div class="mt-3">
          ...
          <!-- 자막없음 -->
          <div v-if="captionList.length === 0" class="text-[15px] font-normal leading-10 mt-0 mb-px mx-0 pt-10 pb-[50px] px-0">자막이 없습니다.</div>
        </div>
      </div>
